...
void vTaskC(void * xTaskParams){
  ...
  while(1){
    ...
    if ( pdTRUE == xQueueReceive(q1, &str2, portMAX_DELAY)) // QueueReceive(q1)
    {
...
void vTaskE(void * xTaskParams){
    ...
    portTickType timePeriod = 600 / portTICK_RATE_MS;
    portTickType timeDiff = xTaskGetTickCount();

    char dataOut[MSG_LEN]=" msg";

  while(1){

    gpioWrite(LEDR,ON);
    vTaskDelay(250 / portTICK_RATE_MS );
    if( pdFALSE == (xQueueSend(q2, &dataOut, portMAX_DELAY )))  // QueueSend(q2)
    {
        printf("Unsent data\r\n");
    }
    gpioWrite(LEDR,OFF);
...
void vTaskF(void * xTaskParams){
  ...
  while(1){
    ...
    if ( pdTRUE == xQueueReceive(q2, &str2, portMAX_DELAY)) // QueueReceive(q2)
    {
      if (pdTRUE == xSemaphoreTake( mutexUART, portMAX_DELAY))
      {
        printf("QueueReceive: [ %s ];\r\n", str2);
